describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItemButton = await page.$('product-item');
    let shadowRootProp = await prodItemButton.getProperty('shadowRoot');
    let buttonClick = await shadowRootProp.$('button');
    await buttonClick.click();
    let innerTextProp = await buttonClick.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 1; i < prodItems.length; i++){
      let shadowRootProp = await prodItems[i].getProperty('shadowRoot');
      let buttonClick = await shadowRootProp.$('button');
      await buttonClick.click();
    }
    const cartCount = await page.$('#cart-count');
    let innerTextProp = await cartCount.getProperty('innerText');
    expect(innerTextProp['_remoteObject'].value).toBe("20"); 
  }, 10000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cartCount = await page.$('#cart-count');
    let innerTextProp = await cartCount.getProperty('innerText');
    expect(innerTextProp['_remoteObject'].value).toBe("0"); 
